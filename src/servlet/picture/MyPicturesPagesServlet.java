...
@WebServlet(name = "MyPicturesPagesServlet")
public class MyPicturesPagesServlet extends HttpServlet {
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int pageNow = Integer.parseInt(request.getParameter("page"));
        List<Picture> pictures = (List<Picture>) request.getSession().getAttribute("myPictures");
        ...
        for (int i = start; i <= end; i++) {
            ...
            result.append("\"><button>查看详情</button></a><a href=\"");
            result.append(request.getContextPath());
            result.append("/upload.jsp?imageID=");
            result.append(picture.getImageID());
            result.append("\"><button>修改</button></a><a onclick=\"return window.confirm('确定要删除吗？')\" href=\"");
            result.append(request.getContextPath());
            result.append("/picture/delete?imageID=");
            result.append(picture.getImageID());
            result.append("\"><button>删除</button></a><div class=\"heat\"><span class=\"fa fa-star\"></span>热度：");
